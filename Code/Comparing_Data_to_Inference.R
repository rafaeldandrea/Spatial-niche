...
## ====== data folder ===========
data_date <- '20200508-80scen-NST20S20-H-allnoise-biotime1e5'
  ...
  crossing(
    ...
    union(
      crossing(
        ...
        mutate(noise  =  find_noise_value(niche_index, seed = run, S = S, NST = NST, option = noise_type)) %>%
        ungroup
    ) %>%
...
if (do.data) {
  ...
  list2env(df_max_loglikelihood, envir = globalenv())
  ...
  RV <- RV_coefficient(true_C, est_C)
  ...
  if (hpcc) {
    ...
    dat <-
      list(
          ...
          seed_initial_community = seed_initial_community,
          ...
          interval = com$interval,
          
          seed = com$seed,
          
          Final_Community = com$Final_Community,
          
          abundances = com$abundances,
          
          times = com$times,
          
          new_deaths = com$new_deaths,
